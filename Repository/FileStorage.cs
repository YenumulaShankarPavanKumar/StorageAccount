...
namespace StorageAccount.Repository
{
    ...
    {
        public static string connectionString="DefaultEndpointsProtocol=https;AccountName=azprojectstorageaccount;AccountKey=PW5B/ifDJS8MocwKAiF6WzqkKPF+92ZJQZZat4eLenidmnU9R2qmgKQgQkhjxv7X/BdqQrHUO/ZE+AStDFhWlw==;EndpointSuffix=core.windows.net";
        ...
        {
            string path=@"C:\AzureStorageAccount\StorageAccount\Download\"+fileName;
            shareServiceClient=new ShareServiceClient(connectionString);
            var serviceClient=shareServiceClient.GetShareClient(fileShareName);
            var dir=serviceClient.GetDirectoryClient(directoryName);
            var file=dir.GetFileClient(fileName);
